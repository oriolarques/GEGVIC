...
![GEGVIC outline](vignettes/GEGVIC_outline.png){width=60%}
...
The genetic variations input (*sample_mutations*) together with samples metadata can be used in the function **gv_mut_summary()** to generate two plots that will first summarise the mutation types present in the samples and second highlight the most common mutations by groups in a form of an oncoplot. 
...
Also, the mutational signature matrices containing the frequencies of all nucleotide changes per signature need to be indicated. GEGVIC contains the matrices from [COSMIC](https://cancer.sanger.ac.uk/signatures/downloads/) for single and double base substitutions. 
