...
2. **Genomic variations data (Muts)**: Table containing short variant calls. Necessary columns **MUST** have the following names (following the [MAF format](https://docs.gdc.cancer.gov/Data/File_Formats/MAF_Format/)):
...
3. **Samples metadata**: Table that contains additional information to the samples to create groups such as response to a therapy. The first column MUST be named **Samples** and contain the same nomenclature for each sample as in the *RNA-sequencing raw counts* and *Genomic variations data tables*. 
